---
name: commit
description: 刷新相关文档后提交到本地并推送到远端（当前分支）。用户输入 /commit 或要求"提 commit"时使用。
whenToUse: 用户输入 /commit、要求提交代码、或要求 commit 并推送到远端时。
---

# /commit — 文档刷新 → 验证 → 确认 → 本地提交 → 推送

DSH 环境版，等价于 `.claude/commands/commit.md`（Claude Code 版）。命令经 pwsh 执行；git 若报 `sh.exe: couldn't create signal pipe`（沙箱拦截），用 `sandbox_permissions: "danger-full-access"` 重试同一条命令。

0. **可选消息**：`/commit <msg>` 提供的消息直接作为提交信息；未提供则自动生成。
...
7. **确认**：把 `git diff --stat` 摘要、文档改动点、生成的提交信息一并展示，等用户明确确认后再提交与推送（黄金规则：push 必须用户明确许可）。
