...
- ➕ **记账** — 底部中央「＋」按钮唤起全屏向导：支出/收入切换 → 选类别 → 自定义数字键盘（支持加减法）输入金额
- 📅 **明细** — 日历视图（支出绿点/收入蓝点）+ 按日清单（-¥ 支出 / +¥ 收入）+ 编辑/删除
- 💰 **预算** — 总预算为主，分类预算从总预算分配（和 ≤ 总预算）；预算执行进度、超支预警
- 📊 **统计** — 周/月/年 + 本月概览卡（支出/收入/结余 + 预算执行）+ 收支切换 + 趋势折线图 + 折叠类别列表
- 👤 **我的** — 语言切换 / 深色模式 / 统计显示设置 / 导出导入 / 类别管理
...
v1.2.0<!-- @audit:version -->（pre-commit 自动同步，源为 src-tauri/tauri.conf.json）
