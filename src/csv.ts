...
// 7 列格式：日期,一级分类,二级分类,金额,备注,记录时间,类型。类型放末列 → 旧 6 列文件的字段位序完全不变，导入兼容
...
export function parseImportCsv(content: string, categories?: CsvCategoryMap): ParsedCsvResult {
  ...
  const headerCols = lines[0].replace(/^﻿/, '').split(',').map((h) => h.trim().toLowerCase());
